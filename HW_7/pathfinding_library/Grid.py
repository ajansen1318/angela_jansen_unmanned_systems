...
class Grid:
    def __init__(
        ...
        self.invalid_indices: set(int) = set()

        self.set_invalid_indicies()
    ...
    def pos_not_valid(
        ...
        pos_index = self.compute_index(x_curr, y_curr)
        return pos_index in self.invalid_indices

    def set_invalid_indicies(self):
        for row in np.arange(self.min_x, self.max_x + self.grid_space, self.grid_space):
            for col in np.arange(
                self.min_y, self.max_y + self.grid_space, self.grid_space
            ):
                pos_invalid = self.pos_in_obstacle(row, col)
                if pos_invalid:
                    self.invalid_indices.add(self.compute_index(row, col))
