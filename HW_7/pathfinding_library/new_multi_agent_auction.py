...
class Agent:
    def __init__(self, i, xi, yi, speed, N_tasks, color, grid: Grid):
        self.i = i  # my index
        self.x = xi  # position
        self.y = yi  # position
    ...
    def find_best_task(self, tasks):
        ...
        # stuff added for astar
        for i, task in enumerate(tasks):
            task_is_active = task.active
            if task_is_active:  # don't do tasks that are done
                path, cost = self.astar_pathfinder((self.x, self.y), (task.x, task.y))
                # cost = (
                #     np.sqrt(pow(task.x - self.x, 2) + pow(task.y - self.y, 2))
                #     / self.speed
                # )  # cost here is time
                # print('task[', i, '].cost: ', task.x, ', ', task.y)

                if cost < min_cost and cost < self.team_bids[i][0]:
                    min_cost = cost
                    min_i = i
                    self.path = path

        self.goal_task = [min_cost, min_i]  # set goal task
        ...
        #         # print('task[', i, '].cost: ', task.x, ', ', task.y)
        #         if (
        #             cost < min_cost and cost < self.team_bids[i][0]
        #         ):  # is cost better than my current best and my team's current best
    ...
    def at_goal(self, world):  # check if I am at my goal
        ...
        return (
            np.sqrt(pow(task.x - self.x, 2) + pow(task.y - self.y, 2)) <= self.speed
        )  # check if I am within 1 timestep

    def move_towards_goal(self, world):  # move towards goal
        # step towards first waypoint in self.path
        if len(self.path) == 0:
            return

        next_x, next_y = self.path[0]
        dx = next_x - self.x  # x dist
        dy = next_y - self.y  # y dist
        dist = np.sqrt(pow(dx, 2) + pow(dy, 2))  # euclid dist

        # if we're close to waypoint, remove it from the path
        if dist < self.speed:
            self.path.pop(0)
            return
    ...
    def advertise_bid(self, agents, world):  # tell my team my plans
        for agent in agents:  # go through all agents
            if agent != self:  # don't update team bids with my bid
                ...
                if (
                    np.sqrt(pow(agent.x - self.x, 2) + pow(agent.y - self.y, 2))
                    < world.coms_range
                ):
    ...
    def update_bid(
        self, task_bid, agent_index
    ):  # remove old bids and update with new bid
    ...
    def astar_pathfinder(self, start, goal):
        start_node = Node(start[0], start[1], 0, -1)
        end_node = Node(goal[0], goal[1], 0, -1)

        astar = DijkstraAstar(self.grid, start_node, end_node, use_dijkstra=False)
        path = astar.find_path()

        wps = [(node.x, node.y) for node in path]
        cost = path[0].cost
        return wps, cost
...
while len(obs_positions) < N_obstacles:
    ...
    obstacles.append(Obstacle(x_obs, y_obs, obs_radius))

    circle = plt.Circle((x_obs, y_obs), obs_radius, color="c", fill=True)
...
for a in range(0, N_agents):
    while True:
        x_pos = round((random.random() * world.x) * 2) / 2
        y_pos = round((random.random() * world.y) * 2) / 2
        if grid.pos_not_valid(x_pos, y_pos):
            continue
        else:
            agents.append(Agent(a, x_pos, y_pos, 2.0, N_tasks, colors[a % 5], grid))
            plt.plot(x_pos, y_pos, colors[a % 5] + "o")
            break
...
for ts in range(0, N_time_steps):  # iterate time
    ...
    for wl in world_log:
        for agent in agents:
            circle = plt.Circle((agent.x, agent.y), 0.1, color=agent.color, fill=True)
            plt.gcf().gca().add_artist(circle)
    ...
    # for agent in agents:
    #     plt.plot(
    #         world_log[0][agent.i * 2], world_log[0][agent.i * 2 + 1], agent.color + "o"
    #     )
